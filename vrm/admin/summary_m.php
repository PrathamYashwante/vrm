...
if (isset($_POST['submit'])) {
    ...
    $filename = 'summary' . $dateInput . '.xls';
    $xlsContent = '';
    ...
    foreach ($output as $table => $dates) {
        ...
        $modifiedTableName = str_replace('_sm', '', $table);
        $dateHeader = "<tr><th style='border: 1px solid #000;'>$modifiedTableName</th>";
        ...
        if ($subtotalColumn) {
            $tableData .= "<tr>";
            $tableData .= "<td style='border: 1px solid #000; text-align: center;'>$subtotalColumn</td>";

            // Iterate over dates
            foreach ($dates as $date => $columns) {
                if (isset($columns[$subtotalColumn])) {
                    $previousValue = $columns[$subtotalColumn][0];
                    $currentValue = $columns[$subtotalColumn][1];
                } else {
                    $previousValue = 0;
                    $currentValue = 0;
                }
    ...
    // Set appropriate headers for the Excel file
...
<div class="container">
    <div class="card">
        ...
        <form method="POST" action="">
            ...
            <button type="submit" name="submit">Retrieve Summary</button>
        </form>
    </div>
</div>